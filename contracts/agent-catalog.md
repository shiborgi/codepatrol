...
instructions. Producer Open requires `--agents reference@version,...`; every entry
is resolved before context retrieval or state mutation, then one envelope is returned
per task under `{ "tasks": [...] }`. Reviews and Ship resolve their single configured
default only. Task Show, Submit, and Retry never resolve again.
