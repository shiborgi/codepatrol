...
export interface KanbanRow { work: string; branch: string; plan: string; review: string; apply: string; verify: string; finalize: string; total: string; nextAction?: string; createdAt: string; workId: string }
...
export function projectKanban(changes: ChangeView[], options: { asOf?: string; all?: boolean } = {}): KanbanBoard {
	...
	const rows = changes.filter((view) => options.all || view.state !== "terminal").sort((a, b) => a.identity.created_at.localeCompare(b.identity.created_at) || a.identity.work_id.localeCompare(b.identity.work_id)).map((view) => {
		...
		return { work: `${view.identity.work_id} ${view.identity.title}`, workId: view.identity.work_id, createdAt: view.identity.created_at, branch: view.identity.branch, plan: stageCell(view, "plan", options.asOf), review: stageCell(view, "review", options.asOf), apply: stageCell(view, "apply", options.asOf), verify: stageCell(view, "verify", options.asOf), finalize: view.outcome ? `${view.outcome === "committed" ? "commit" : "rollback"} ${view.terminalCommit?.slice(0, 12)}` : stageCell(view, "finalize", options.asOf), total: `${tokenText(total)}; active ${duration(total.activeMs)}; cycle ${duration(cycleMs)}`, ...(view.nextAction ? { nextAction: view.nextAction } : {}) };
	});
...
export function renderKanbanMarkdown(board: KanbanBoard): string {
	const header = "| Work | Branch | Plan | Review | Apply | Verify | Finalize | Total |\n|---|---|---|---|---|---|---|---|\n";
	return header + board.rows.map((row) => `| ${escape(row.work)}${row.nextAction ? `<br>next: ${escape(row.nextAction)}` : ""} | ${escape(row.branch)} | ${escape(row.plan)} | ${escape(row.review)} | ${escape(row.apply)} | ${escape(row.verify)} | ${escape(row.finalize)} | ${escape(row.total)} |`).join("\n") + (board.rows.length ? "\n" : "");
}
