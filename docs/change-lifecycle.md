...
| Verify | `commit` | Finalize | implementation defect → Apply; contract defect → Plan |
| Finalize | `committed` or `rolled-back` | terminal | target advance → stop |
...
and exact expected refs. Verify binds candidate commit/tree. Finalize checks the
...
Only a later explicit Finalize/cutover instruction may fast-forward `main`,
